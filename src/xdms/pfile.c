...
static USHORT Process_Track(FILE *, /* FILE *, */ UCHAR *, UCHAR *, UCHAR *, USHORT, USHORT, USHORT);
...
USHORT Process_File(char *iname, 
                    UCHAR *fbuf, 
                    USHORT cmd, USHORT opt,
                    USHORT PCRC, USHORT pwd)
{
	FILE *fi;
	...
	if ( (cmd == CMD_VIEW) || (cmd == CMD_VIEWFULL) ) {
		...
		/*  not supported for compression. It was for future expansion and was never  */
	...
	if (cmd == CMD_UNPACK) 
	{			
		/* fbuf = (UCHAR*) calloc ((size_t) 901120, 1); */
	}
	...
	if (cmd != CMD_VIEW) {
		...
		{
			/* unpacking loop: unpack first to last track (sequentially) */
			while ( (ret=Process_Track(fi, fbuf, b1, b2, cmd, 
			                           opt,(geninfo & 2) ? pwd : 0)) == NO_PROBLEM );
			
			if ((cmd == CMD_UNPACK) && (opt == OPT_VERBOSE))
				fprintf(stderr,"\n");
		}
	}

	if ((cmd == CMD_VIEWFULL) || (cmd == CMD_SHOWBANNER)) fprintf(stdout, "\n");
	...
	if ((ret == FILE_END) || (ret == ERR_NOTTRACK))
		  ret = NO_PROBLEM;
...
static USHORT Process_Track(FILE *fi, UCHAR *fbuf,
                            UCHAR *b1, UCHAR *b2, 
                            USHORT cmd, USHORT opt, USHORT pwd)
{
  ...
  /* declare STATIC so we can remember old pos! */
  static ULONG mempos; 
	...
	if ((pklen1 > TRACK_BUFFER_LEN) || 
		  (pklen2 > TRACK_BUFFER_LEN) || 
		  (unpklen > TRACK_BUFFER_LEN)) 
		    return ERR_BIGTRACK;
  ...
  /* regular unpacking */
	...
	/* if we arrive here, everything has been OK so far */			
	
	/* debug stuff */	 
	/* printf ("[DEBUG] mempos = %ld      mempos + unpklen = %ld\n", mempos, mempos+unpklen); */
	
	memcpy (fbuf+mempos, b2, (size_t)unpklen);	
	mempos += unpklen;
